        ...
        <ul className="text-left overflow-auto mt-2">
          {
            recentHistory && recentHistory.map((item,index)=>(
              <li key={index}
              onClick={()=>setselectedHistory(item)} className="p-1 pl-5 px-5 truncate dark:text-zinc-400  text-zinc-700 cursor-pointer dark:hover:bg-zinc-700 hover:bg-red-200 hover-text-zinc-200 dark:hover:text-zinc-200">{item}</li>
            ))
          }
        </ul>
      </div>
